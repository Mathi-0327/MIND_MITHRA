...
export interface MindMithraLogoProps {
...
export const MindMithraLogo: React.FC<MindMithraLogoProps> = ({
  size = 'md',
  showTagline = true,
  theme = 'amber',
  animated = false,
  ...
  return (
    <div className={`inline-flex items-center ${currentSize.gap} ${className}`} id="mind-mithra-app-logo">
      ...
      <div
        ...
        style={{
          background: 'linear-gradient(135deg, #58745A 0%, #B28A32 50%, #C66F4E 100%)',
          boxShadow: '0 8px 24px -4px rgba(88, 116, 90, 0.45), inset 0 1px 1px rgba(255, 255, 255, 0.4)',
        }}
      >
        {/* Glow Ring behind logo */}
        <div className="absolute inset-0 rounded-2xl sm:rounded-3xl bg-amber-400/25 blur-md -z-10 animate-pulse" />

        {/* Mind & Lotus Harmony Vector */}
        <svg
          viewBox="0 0 40 40"
          fill="none"
      ...
      <div className="flex flex-col text-left leading-none">
        <div className="flex items-center gap-1.5">
          ...
          <span
            className={`px-1.5 py-0.5 rounded-md font-extrabold uppercase text-[9px] tracking-wider shadow-xs ${
              theme === 'dark'
        ...
        {showTagline && (
          <span
            className={`font-semibold tracking-wide mt-1 uppercase ${currentSize.sub} ${
              theme === 'dark' || theme === 'amber'
                ? 'text-amber-200/80'
                : 'text-stone-500'
            }`}
          >
            Cultural Cognitive Care &amp; Reminiscence
          </span>
        )}
      </div>
    </div>
...
export const ManasLogo = MindMithraLogo;
export type ManasLogoProps = MindMithraLogoProps;
